...
public class MixinPlayerControllerMP
{
    ...
    public float getPlayerRelativeBlockHardnessHook(final IBlockState state, final EntityPlayer player, final World worldIn, final BlockPos pos) {
        return state.getPlayerRelativeBlockHardness(player, worldIn, pos) * ((TpsSync.getInstance().isOn() && (boolean)TpsSync.getInstance().mining.getValue()) ? (1.0f / Phobos.serverManager.getTpsFactor()) : 1.0f);
    }
    ...
    private void getReachDistanceHook(final CallbackInfoReturnable<Float> distance) {
        if (Reach.getInstance().isOn()) {
            ...
            distance.setReturnValue((Object)(Reach.getInstance().override.getValue() ? Reach.getInstance().reach.getValue() : Float.valueOf(range + (float)Reach.getInstance().add.getValue())));
        }
    }
    ...
    public boolean canPlaceBlockOnSideHook(final ItemBlock itemBlock, final World worldIn, BlockPos pos, EnumFacing side, final EntityPlayer player, final ItemStack stack) {
        ...
        if (axisalignedbb != Block.NULL_AABB && !worldIn.checkNoEntityCollision(Objects.requireNonNull(axisalignedbb).offset(pos), (Entity)null)) {
