...
public class BowSpam extends Module
{
    ...
    public void onUpdateWalkingPlayer(final UpdateWalkingPlayerEvent event) {
        ...
        if (this.mode.getValue() == Mode.FAST && (this.offhand || BowSpam.mc.player.inventory.getCurrentItem().getItem() instanceof ItemBow) && BowSpam.mc.player.isHandActive()) {
            ...
            final float f3 = this.tpsSync.getValue() ? Phobos.serverManager.getTpsFactor() : 1.0f;
            if (f >= f2 * f3) {
    ...
    public void onUpdate() {
        ...
        switch (this.mode.getValue()) {
            case AUTORELEASE: {
                ...
                if (!this.timer.passedMs((int)(this.delay.getValue() * (this.tpsSync.getValue() ? Phobos.serverManager.getTpsFactor() : 1.0f)))) {
                    break;
                }
    ...
    private EntityPlayer getTarget() {
        ...
        for (final EntityPlayer player : BowSpam.mc.world.playerEntities) {
            if (player != null && !EntityUtil.isDead((Entity)player) && EntityUtil.getHealth((Entity)player) <= this.health.getValue() && !player.equals((Object)BowSpam.mc.player) && !Phobos.friendManager.isFriend(player) && BowSpam.mc.player.getDistanceSq((Entity)player) <= MathUtil.square(this.range.getValue())) {
