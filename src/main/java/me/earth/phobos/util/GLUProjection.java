...
public final class GLUProjection
{
    ...
    public void updateMatrices(final IntBuffer viewport,  final FloatBuffer modelview,  final FloatBuffer projection,  final double widthScale,  final double heightScale) {
        ...
        final double pitch = ((-fv.y > 0.0 && yaw >= 90.0 && yaw < 270.0) || (fv.y > 0.0 && (yaw < 90.0 || yaw >= 270.0))) ? Math.toDegrees(Math.atan2(nuv.cross(uv).length(),  nuv.dot(uv))) : (-Math.toDegrees(Math.atan2(nuv.cross(uv).length(),  nuv.dot(uv))));
        ...
        if (outsideFrustum) {
            ...
            if ((extrudeInverted && !outsideInvertedFrustum) || (outsideInvertedFrustum && clampModeOutside != ClampMode.NONE)) {
                if ((extrudeInverted && !outsideInvertedFrustum) || clampModeOutside == ClampMode.DIRECT) {
                    ...
                    final Vector3D intersect = (angle >= this.bra && angle < this.tra) ? this.rb.intersect(vectorLine) : ((angle >= this.tra && angle < this.tla) ? this.tb.intersect(vectorLine) : ((angle >= this.tla && angle < this.bla) ? this.lb.intersect(vectorLine) : this.bb.intersect(vectorLine)));
