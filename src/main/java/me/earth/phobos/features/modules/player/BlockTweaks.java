...
extends Module {
    ...
    public void onPacketSend(PacketEvent.Send event) {
        ...
        if (this.noFriendAttack.getValue().booleanValue() && event.getPacket() instanceof CPacketUseEntity && (entity = ((CPacketUseEntity)event.getPacket()).func_149564_a((World)BlockTweaks.mc.field_71441_e)) != null && Phobos.friendManager.isFriend(entity.func_70005_c_())) {
    ...
    private void equipBestTool(IBlockState blockState) {
        ...
        for (int i = 0; i < 9; ++i) {
            ...
            if (!(f > 1.0f) || !((double)(speed = (float)((double)speed + ((eff = EnchantmentHelper.func_77506_a((Enchantment)Enchantments.field_185305_q, (ItemStack)stack)) > 0 ? Math.pow(eff, 2.0) + 1.0 : 0.0))) > max)) continue;
            max = speed;
            bestSlot = i;
        }
