...
public class OffhandRewrite extends Module
{
    ...
    public void doSwitch() {
        ...
        if (this.antiPing.getValue() && (PingBypass.getInstance().isConnected() ? PingBypass.getInstance().getServerPing() : Phobos.serverManager.getPing()) > this.pingvalue.getValue()) {
            this.setMode(Mode2.TOTEMS);
        }
        if (this.lagSwitch.getValue() && Phobos.serverManager.isServerNotResponding()) {
            this.setMode(Mode2.TOTEMS);
        }
