...
extends Module {
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        if (this.renderingUp.getValue().booleanValue()) {
            ...
            if (this.potions.getValue().booleanValue()) {
                for (PotionEffect effect : Phobos.potionManager.getOwnPotions()) {
                    ...
                    int n3 = height - 2;
                    renderer3.drawString(text3, x3, n3 - (k += 10), this.rolling.getValue() != false && this.rainbow.getValue() != false ? this.colorMap.get(height - k) : (this.altPotionsColors.getValue() != false ? this.potionColorMap.get((Object)effect.func_188419_a()).getRGB() : this.color), true);
                }
            }
        ...
        if (this.friendslist.getValue().booleanValue()) {
            ...
            int n3 = 30;
            for (Entity entity : HUD.mc.field_71441_e.field_73010_i) {
                if (!Phobos.friendManager.isFriend(entity.func_70005_c_())) continue;
                this.renderer.drawString("Friends", 1.0f, 30.0f, object.getRGB(), true);
                this.renderer.drawString(entity.func_70005_c_(), 1.0f, n3 += this.renderer.getFontHeight() + 1, object.getRGB(), true);
            }
        }
    }
