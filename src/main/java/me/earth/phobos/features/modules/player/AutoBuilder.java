...
extends Module {
    ...
    public void onEnable() {
        ...
        if (!this.keepPos.getValue().booleanValue() || this.startPos == null) {
            this.startPos = new BlockPos(AutoBuilder.mc.field_71439_g.field_70165_t, Math.ceil(AutoBuilder.mc.field_71439_g.field_70163_u), AutoBuilder.mc.field_71439_g.field_70161_v).func_177977_b();
        }
        this.blocksThisTick = 0;
        this.lastSlot = AutoBuilder.mc.field_71439_g.field_71071_by.field_70461_c;
        this.timer.reset();
    }
    ...
    private void doAutoBuilder() {
        ...
        for (BlockPos pos : this.placepositions) {
            ...
            if (BlockUtil.isPositionPlaceable(pos.func_177977_b(), false, true) == 3) {
                BlockUtil.placeBlockNotRetarded(pos.func_177977_b(), EnumHand.MAIN_HAND, this.rotate.getValue(), this.packet.getValue(), this.altRotate.getValue());
                ++this.blocksThisTick;
                continue;
            }
            switch (this.stairDirection.getValue()) {
                case SOUTH: {
                    if (BlockUtil.isPositionPlaceable(pos.func_177968_d(), false, true) != 3) break;
                    BlockUtil.placeBlockNotRetarded(pos.func_177968_d(), EnumHand.MAIN_HAND, this.rotate.getValue(), this.packet.getValue(), this.altRotate.getValue());
                    ++this.blocksThisTick;
                    break;
                }
                case WEST: {
                    if (BlockUtil.isPositionPlaceable(pos.func_177976_e(), false, true) != 3) break;
                    BlockUtil.placeBlockNotRetarded(pos.func_177976_e(), EnumHand.MAIN_HAND, this.rotate.getValue(), this.packet.getValue(), this.altRotate.getValue());
                    ++this.blocksThisTick;
                    break;
                }
                case NORTH: {
                    if (BlockUtil.isPositionPlaceable(pos.func_177978_c(), false, true) != 3) break;
                    BlockUtil.placeBlockNotRetarded(pos.func_177978_c(), EnumHand.MAIN_HAND, this.rotate.getValue(), this.packet.getValue(), this.altRotate.getValue());
                    ++this.blocksThisTick;
                    break;
                }
                case EAST: {
                    if (BlockUtil.isPositionPlaceable(pos.func_177974_f(), false, true) != 3) break;
                    BlockUtil.placeBlockNotRetarded(pos.func_177974_f(), EnumHand.MAIN_HAND, this.rotate.getValue(), this.packet.getValue(), this.altRotate.getValue());
                    ++this.blocksThisTick;
                }
            }
        }
    ...
    private boolean doSwitch(boolean back) {
        Item i = AutoBuilder.mc.field_71439_g.func_184614_ca().func_77973_b();
        switch (this.switchMode.getValue()) {
            case NONE: {
                if (i instanceof ItemBlock) {
                    ...
                    return ((ItemBlock)i).func_179223_d() instanceof BlockObsidian;
                }
                return false;
            }
            case NORMAL: {
            ...
            case SILENT: {
                if (i instanceof ItemBlock && (this.allBlocks.getValue().booleanValue() || ((ItemBlock)i).func_179223_d() instanceof BlockObsidian) || this.lastSlot == -1) break;
                if (back) {
                    AutoBuilder.mc.field_71439_g.field_71174_a.func_147297_a((Packet)new CPacketHeldItemChange(this.lastSlot));
                    break;
                }
                AutoBuilder.mc.field_71439_g.field_71174_a.func_147297_a((Packet)new CPacketHeldItemChange(this.blockSlot));
            }
        }
        return true;
    }
    ...
    private boolean check() {
        ...
        if (this.ground.getValue().booleanValue() && !AutoBuilder.mc.field_71439_g.field_70122_E) {
            return false;
        }
        if (this.blockSlot == -1) {
            return false;
        }
        if (this.noMove.getValue().booleanValue() && (AutoBuilder.mc.field_71439_g.field_191988_bg != 0.0f || AutoBuilder.mc.field_71439_g.field_70702_br != 0.0f)) {
            return false;
        }
    ...
    private void getPositions() {
        ...
        this.placepositions.clear();
        for (BlockPos pos : BlockUtil.getSphere(new BlockPos(AutoBuilder.mc.field_71439_g.field_70165_t, Math.ceil(AutoBuilder.mc.field_71439_g.field_70163_u), AutoBuilder.mc.field_71439_g.field_70161_v).func_177984_a(), this.range.getValue().floatValue(), this.range.getValue().intValue(), false, true, 0)) {
            if (this.placepositions.contains((Object)pos) || !AutoBuilder.mc.field_71441_e.func_175623_d(pos)) continue;
            if (this.mode.getValue() == Mode.STAIRS) {
                switch (this.stairDirection.getValue()) {
                    case NORTH: {
                        if (this.startPos.func_177952_p() - pos.func_177952_p() != pos.func_177956_o() - this.startPos.func_177956_o() || Math.abs(pos.func_177958_n() - this.startPos.func_177958_n()) >= this.width.getValue() / 2) break;
                        this.placepositions.add(pos);
                        break;
                    }
                    case EAST: {
                        if (pos.func_177958_n() - this.startPos.func_177958_n() != pos.func_177956_o() - this.startPos.func_177956_o() || Math.abs(pos.func_177952_p() - this.startPos.func_177952_p()) >= this.width.getValue() / 2) break;
                        this.placepositions.add(pos);
                        break;
                    }
                    case SOUTH: {
                        if (pos.func_177952_p() - this.startPos.func_177952_p() != pos.func_177956_o() - this.startPos.func_177956_o() || Math.abs(this.startPos.func_177958_n() - pos.func_177958_n()) >= this.width.getValue() / 2) break;
                        this.placepositions.add(pos);
                        break;
                    }
                    case WEST: {
                        if (this.startPos.func_177958_n() - pos.func_177958_n() != pos.func_177956_o() - this.startPos.func_177956_o() || Math.abs(this.startPos.func_177952_p() - pos.func_177952_p()) >= this.width.getValue() / 2) break;
                        this.placepositions.add(pos);
                    }
                }
                continue;
            }
            if (this.mode.getValue() != Mode.FLAT || (double)pos.func_177956_o() != (this.dynamic.getValue() != false ? Math.ceil(AutoBuilder.mc.field_71439_g.field_70163_u) - 1.0 : (double)this.startPos.func_177956_o())) continue;
            this.placepositions.add(pos);
        }
    }
