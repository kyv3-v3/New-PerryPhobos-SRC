    ...
    public void execute(String[] commands) {
        ...
        if (commands.length == 2) {
            if ("list".equals(commands[0])) {
                ...
                List directories = Arrays.stream((Object[])Objects.requireNonNull(file.listFiles())).filter(File::isDirectory).filter(f -> !f.getName().equals("util")).collect(Collectors.toList());
